...
from trendradar.storage.rss_sqlite_mixin import (
    RSSStorageMixin,
    OFFICIAL_CHANGE_FEED_ID,
    OFFICIAL_CHANGE_CHECKPOINT_DATE,
    CHANGE_STATUSES,
)
from trendradar.storage.tags_sqlite_mixin import TagsStorageMixin
from trendradar.utils.url import normalize_url


class SQLiteStorageMixin(RSSStorageMixin, TagsStorageMixin):
    """
    SQLite 存储操作 Mixin
    ...
    def _migrate_rss_schema(self, conn: sqlite3.Connection) -> None:
        ...
        migrations = {
            ...
            "deactivated_at": "TEXT",
            "revision_crawl_time": "TEXT DEFAULT ''",
            "ai_sync_pending": "INTEGER DEFAULT 0",
        }
        for column, definition in migrations.items():
            if column not in columns:
                conn.execute(f"ALTER TABLE rss_items ADD COLUMN {column} {definition}")
        ...
        """)
        conn.execute("""
            CREATE UNIQUE INDEX IF NOT EXISTS idx_rss_change_feed
            ON rss_items(change_id, feed_id) WHERE change_id != ''
        """)
        conn.execute("""
            CREATE INDEX IF NOT EXISTS idx_rss_active_feed
            ON rss_items(feed_id, is_active)
        """)
        conn.execute("""
        ...
        if "delivery_state" not in checkpoint_columns:
            # 旧版本在检测时就把记录视为已发布；迁移时保留该语义，
            # 避免升级后把全部历史变化重新推送一遍。
            conn.execute("""
                ALTER TABLE official_change_checkpoint
                ADD COLUMN delivery_state TEXT NOT NULL DEFAULT 'delivered'
            """)
        if "delivered_at" not in checkpoint_columns:
            conn.execute("""
                ALTER TABLE official_change_checkpoint
                ADD COLUMN delivered_at TEXT
            """)
        conn.execute("""
            CREATE INDEX IF NOT EXISTS idx_official_change_delivery
            ON official_change_checkpoint(delivery_state, status)
        """)
